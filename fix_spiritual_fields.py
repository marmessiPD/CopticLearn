#!/usr/bin/env python3
"""
Fix spiritual fields in module1.ts to match the TypeScript interface requirements.
"""
...
# Define the fixes for each spiritual section
fixes = [
    # Alpha (lines ~400-416)
    {
        'old': '''          fatherQuote: {
    ...
    # Beta/Vita (lines ~427-435)
    {
        'old': '''          bibleVerse: { de: '„Die Leuchte des Leibes ist das Auge."' },
    ...
    # Gamma (lines ~446-454)
    {
        'old': '''          bibleVerse: { de: '„Denn so sehr hat Gott die Welt geliebt…"' },
    ...
    # Delta (lines ~465-473)
    {
        'old': '''          bibleVerse: { de: '„…ein Mann nach dem Herzen Gottes."' },
...
for fix in fixes:
    if fix['old'] in content:
        ...
        count += 1
        print(f"[OK] Applied fix {count}")
    else:
        print(f"[SKIP] Could not find text to replace for fix {count + 1}")

# Write the file back
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"\n{count} fixes have been applied!")
