...
const PronunciationRuleCard = lazy(() => import('../components/PronunciationRuleCard'));
                        ...
                        <div className="space-y-4">
                            {exerciseSlide.tasks?.map((task: any, idx: number) => (
                                <div key={idx} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                    ...
                                    <p className="text-coptic-blue dark:text-coptic-gold font-coptic text-xl">{task.answer}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                return (
                    <div className="bg-light-primary dark:bg-dark-secondary rounded-xl shadow-lg p-4 sm:p-6 max-w-2xl mx-auto">
                        <div className="text-center mb-4">
                            {theologySlide.meta?.emoji && <span className="text-4xl mb-2 block">{theologySlide.meta.emoji}</span>}
    ...
    return (
        <div className="container mx-auto max-w-4xl px-2 sm:px-4">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
                <Link to={`/module/${lesson.moduleId}`} className="text-coptic-blue dark:text-coptic-gold hover:underline text-sm sm:text-base order-2 sm:order-1">
                    &larr; {t({de: "Zurück zum Modul", en: "Back to Module", ar: "العودة إلى الوحدة"})}
                </Link>
                <h2 className="text-lg sm:text-2xl font-bold text-center order-1 sm:order-2">{t(lesson.title)}</h2>
            ...
            <div className="relative" ref={slideContainerRef}>
                ...
                <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-1 opacity-50">
                    ...
                    <div className="text-xs text-gray-400 dark:text-gray-500">
                        {t({de: "oder Pfeiltasten", en: "or arrow keys", ar: "أو مفاتيح الأسهم"})}
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
                ...
                >
                    <ArrowLeftIcon className="h-5 w-5" /> {t({de: "Zurück", en: "Previous", ar: "السابق"})}
                </button>

                <span className="text-sm sm:text-base text-center">{t({de: "Folie", en: "Slide", ar: "شريحة"})} {currentSlideIndex + 1} / {lesson.slides.length}</span>
                ...
                {isLastSlide ? (
                    ...
                    >
                        {t({de: "Zum Quiz", en: "Go to Quiz", ar: "إلى الاختبار"})} <ArrowRightIcon className="h-5 w-5" />
                    </button>
                ) : (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-3 bg-coptic-blue text-white rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed dark:bg-coptic-gold dark:text-coptic-blue dark:disabled:bg-gray-600 min-h-[44px] touch-manipulation w-full sm:w-auto"
                    >
                       {t({de: "Weiter", en: "Next", ar: "التالي"})} <ArrowRightIcon className="h-5 w-5" />
                    </button>
                )}
            </div>
        </div>
