import { DISABLE_AUDIO } from "../constants/featureFlags";
import React from "react";
import { ConversationSlide } from "../types";
import { useAppContext } from "../context/AppContext";

const SpeakerIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className="h-5 w-5"
    viewBox="0 0 20 20"
    fill="currentColor"
    {...props}
  >
    <path
      fillRule="evenodd"
      d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.414z"
      clipRule="evenodd"
    />
  </svg>
);

interface ConversationCardProps {
  slide: ConversationSlide;
}

const ConversationCard: React.FC<ConversationCardProps> = ({ slide }) => {
  const { t, playSound } = useAppContext();

  return (
    <div className="bg-light-primary dark:bg-dark-secondary rounded-xl shadow-lg p-4 sm:p-6 max-w-2xl mx-auto">
      {/* Header */}
      <div className="text-center mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-coptic-blue dark:text-coptic-gold">
          {t(slide.title)}
        </h2>
        {slide.participants && slide.participants.length > 0 && (
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-2">
            {slide.participants.join(" • ")}
          </p>
        )}
      </div>

      {/* Conversation Lines */}
      <div className="space-y-4">
        {slide.lines.map((line, index) => (
          <div
            key={index}
            className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg"
          >
            {/* Speaker */}
            <div className="mb-2">
              <span className="text-sm sm:text-base font-semibold text-coptic-blue dark:text-coptic-gold">
                {line.speaker ? t(line.speaker) : ''}:
              </span>
            </div>

            {/* Coptic Text */}
            <div className="mb-2">
              <p className="text-2xl sm:text-3xl font-coptic text-gray-800 dark:text-gray-200">
                {line.coptic}
              </p>
            </div>

            {/* Transliteration */}
            <div className="mb-2">
              <p className="text-sm sm:text-base italic text-gray-600 dark:text-gray-400">
                {line.translit}
              </p>
            </div>

            {/* Meaning */}
            <div className="flex items-start justify-between gap-2">
              <p className="text-base sm:text-lg font-semibold flex-1">
                {line.meaning ? t(line.meaning) : ''}
              </p>
              <button
                style={{ display: DISABLE_AUDIO ? "none" : undefined }}
                onClick={() => !DISABLE_AUDIO && playSound(line.coptic)}
                className="p-2 rounded-full text-coptic-blue dark:text-coptic-gold hover:bg-coptic-blue/10 dark:hover:bg-coptic-gold/10 flex-shrink-0 min-h-[44px] min-w-[44px] flex items-center justify-center"
                aria-label={t({
                  de: "Zeile anhÃ¶ren",
                  en: "Listen to line",
                  ar: "Ø§Ø³ØªÙ…Ø¹ Ù„Ù„Ø³Ø·Ø±",
                })}
              >
                <SpeakerIcon />
              </button>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default ConversationCard;

